...
// Modified copy of Francisco Montanez RadixUI Generator to create the bindings
// for shadcn components that are based on RadixUI components
...
module Render =
    ...
    let renderComponent radixComponent =
        [
            ...
            for subComponent in radixComponent.SubComponents do
                ...
                // Shadcn components based on radix ui components can directly use the bindings of radix ui
                // We will modify the npm import strategy to instead reference the project directory
                // Users will have to be instructed on where to install the components to use them
                let npmPackage =
                    radixComponent.NpmPackage
                    |> _.Substring( "@radix-ui/react-".Length )
                    |> (+) "@/components/ui/" // Use the same convention as Shadcn
                $"/// {subComponent.Description}" |> indent4
                $"static member inline {subComponent.Name |> lowerFirst |> appendApostropheToReservedKeywords} (props: IReactProperty seq) = createElement (import \"{importName |> removeSpaces}\" \"{npmPackage}\") props" |> indent4
            ""
            ""
        ]
    ...
    let renderComponentPage radixComponent =
        System.IO.File.WriteAllText($"../Feliz.Shadcn/{radixComponent.Name |> removeSpaces}.fs",
            [
                $"namespace rec Feliz.Shadcn.{radixComponent.Name |> removeSpaces}"
                ""
                "open Feliz"
                "open Fable.Core"
